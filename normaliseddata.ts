const posts = [
  ...
  function normalizeData (unnormalizedData: DataById[]): NormalizeData{
    const normalData: NormalizeData = {byId: {}, allIds: []}
    unnormalizedData.map((item)=>{
        normalData.byId[item.id]={id: item.id, title: item.title, body: item.body}
        normalData.allIds.push(item.id)
    })
    return normalData
  };
